...
from services.ingest.chunk_semantics import (
...
CHUNK_INTERPRETER_PROMPT_VERSION = CHUNK_SEMANTICS_PROMPT_SET_VERSION
...
def _resolve_enrichment_client() -> AzureLLMClient:
    if not _env_enabled("AGENTIC_ENRICHMENT_LLM_ENABLED", default=False):
    ...
    return build_chunk_semantics_client()
...
def _chunk_assertion(
    *,
    ...
    page: Dict[str, Any],
    document: Dict[str, Any],
    chunk_projection: Optional[Dict[str, Any]],
    llm_payload: Dict[str, Any],
    proposition: Optional[Dict[str, Any]] = None,
    proposition_index: int = 0,
) -> Dict[str, Any]:
    text = str(paragraph.get("text", "") or "")
    semantic_payload = proposition if isinstance(proposition, dict) else llm_payload
    modality = _modality_from_chunk(text, semantic_payload)
    subject_type, subject_text = _subject_from_chunk(paragraph, text, modality, semantic_payload)
    object_type, object_text = _object_from_chunk(paragraph, text, modality, semantic_payload)
    relation_type = _normalize_key(str(semantic_payload.get("relation_type", ""))) or MODALITY_TO_RELATION.get(modality, "governs")
    action = _extract_action(text, semantic_payload)
    condition_text = None
    if proposition and isinstance(proposition.get("conditions"), list):
        condition_text = "; ".join(_uniq(str(item) for item in proposition.get("conditions", []) if str(item).strip()))[:180] or None
    if not condition_text:
        condition_text = _extract_condition(text, semantic_payload)
    exception_text = None
    if proposition and isinstance(proposition.get("exceptions"), list):
        exception_text = "; ".join(_uniq(str(item) for item in proposition.get("exceptions", []) if str(item).strip()))[:180] or None
    ...
    evidence = {
        ...
        "page_numbers_0": [int(page.get("page_num", 0) or 0)] if page.get("page_num") is not None else [],
        "page_numbers_1": [int(page.get("page_num", 0) or 0) + 1] if page.get("page_num") is not None else [],
        "paragraph_id": paragraph.get("paragraph_id"),
    ...
    if isinstance(chunk_projection, dict) and chunk_projection.get("document_field_evidence"):
        evidence["document_field_evidence"] = chunk_projection.get("document_field_evidence")
    properties = {
        ...
        "evidence": evidence,
        "polarity": str((proposition or {}).get("polarity", "")) or "affirmative",
        "dense_paraphrase": str((proposition or {}).get("dense_paraphrase", ""))[:320],
        "direct_answer": (proposition or {}).get("direct_answer") if isinstance((proposition or {}).get("direct_answer"), dict) else {},
    }
    confidence = 0.45
...
def _chunk_assertions(
    *,
    ...
    propositions = llm_payload.get("propositions")
...
def _chunk_validator_step(assertions: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    normalized = [_validate_assertion(assertion) for assertion in assertions if isinstance(assertion, dict)]
    return normalized, _stage_result(
        status="completed",
        role="chunk_validator",
        payload={
            "assertion_ids": [item.get("assertion_id") for item in normalized],
            "assertion_count": len(normalized),
            "modalities": _uniq(str(item.get("modality", "")) for item in normalized),
        },
    )
...
def _projection_updates(
    *,
    ...
    assertion_ids = [str(assertion.get("assertion_id")) for assertion in assertions if str(assertion.get("assertion_id", "")).strip()]
    modalities = _uniq(str(assertion.get("modality", "")) for assertion in assertions if str(assertion.get("modality", "")).strip())
    paragraph_update["agentic_status"] = "completed" if assertions else "skipped"
    paragraph_update["agentic_assertion_ids"] = _uniq([*paragraph_update.get("agentic_assertion_ids", []), *assertion_ids])
    paragraph_update["agentic_modalities"] = _uniq([*paragraph_update.get("agentic_modalities", []), *modalities])
    paragraph_update["agentic_last_enriched_at"] = _utcnow().isoformat()
    paragraph_update["agentic_payload"] = {
        "assertion_ids": assertion_ids,
        "semantic_dense_summary": semantic_payload.get("semantic_dense_summary"),
        "semantic_query_terms": semantic_payload.get("semantic_query_terms", []),
        "propositions": semantic_payload.get("propositions", []),
        "source_page_ids": _uniq(
            str(assertion.get("source_page_id", ""))
            for assertion in assertions
            if str(assertion.get("source_page_id", "")).strip()
        ),
        "assertion_evidence": {
            str(assertion.get("assertion_id")): ((assertion.get("properties") or {}).get("evidence") if isinstance(assertion.get("properties"), dict) else {})
            for assertion in assertions
            if str(assertion.get("assertion_id", "")).strip()
        },
    }
    paragraph_update["llm_status"] = "completed" if semantic_payload else "skipped"
    paragraph_update["llm_summary"] = str(semantic_payload.get("semantic_dense_summary", "")).strip() or None
    paragraph_update["llm_section_type"] = (
        str(semantic_payload.get("section_kind_case", "") or semantic_payload.get("section_kind", "") or semantic_payload.get("provision_kind", "")).strip()
        or None
    )
    paragraph_update["llm_tags"] = list(semantic_payload.get("semantic_query_terms", [])) if isinstance(semantic_payload.get("semantic_query_terms"), list) else []
    paragraph_update["llm_payload"] = semantic_payload if isinstance(semantic_payload, dict) else {}
    paragraph_update["llm_model"] = None
    paragraph_update["llm_error"] = None
    paragraph_update["llm_updated_at"] = _utcnow().isoformat()
    active_keywords: List[str] = []
    for assertion in assertions:
        relation_key = _normalize_key(str(assertion.get("relation_type", "")))
        modality_key = _normalize_key(str(assertion.get("modality", "")))
    ...
    chunk_projection_update["search_keywords"] = _uniq(
        [
            *chunk_projection_update.get("search_keywords", []),
            *active_keywords,
            *([str(item) for item in semantic_payload.get("semantic_query_terms", [])] if isinstance(semantic_payload.get("semantic_query_terms"), list) else []),
        ]
    )[:24]
    chunk_projection_update["exact_terms"] = _uniq(
        [
            *chunk_projection_update.get("exact_terms", []),
            *([str(item) for item in semantic_payload.get("semantic_query_terms", [])] if isinstance(semantic_payload.get("semantic_query_terms"), list) else []),
        ]
    )[:24]
    chunk_projection_update["edge_types"] = _uniq(
        [*chunk_projection_update.get("edge_types", []), *["refers_to" for assertion in assertions if assertion.get("citation_refs")]]
    )[:12]
    chunk_projection_update["semantic_dense_summary"] = str(semantic_payload.get("semantic_dense_summary", "")).strip() or None
    chunk_projection_update["semantic_query_terms"] = list(semantic_payload.get("semantic_query_terms", [])) if isinstance(semantic_payload.get("semantic_query_terms"), list) else []
    chunk_projection_update["semantic_assertions"] = [
        {
            ...
            "evidence": ((assertion.get("properties") or {}).get("evidence") if isinstance(assertion.get("properties"), dict) else {}) or {},
            "direct_answer": ((assertion.get("properties") or {}).get("direct_answer") if isinstance(assertion.get("properties"), dict) else {}) or {},
            "dense_paraphrase": ((assertion.get("properties") or {}).get("dense_paraphrase") if isinstance(assertion.get("properties"), dict) else "") or "",
            "polarity": ((assertion.get("properties") or {}).get("polarity") if isinstance(assertion.get("properties"), dict) else "affirmative") or "affirmative",
        }
        for assertion in assertions
    ]
    chunk_projection_update["semantic_assertion_count"] = len(assertions)
    chunk_projection_update["semantic_assertion_source_page_ids"] = _uniq(
        str(assertion.get("source_page_id", ""))
        for assertion in assertions
        if str(assertion.get("source_page_id", "")).strip()
    )
    chunk_projection_update["direct_answer_candidates"] = [
        ((assertion.get("properties") or {}).get("direct_answer") if isinstance(assertion.get("properties"), dict) else {})
        for assertion in assertions
        if isinstance((assertion.get("properties") or {}).get("direct_answer"), dict)
        and ((assertion.get("properties") or {}).get("direct_answer") or {}).get("eligible")
    ]
    if chunk_projection_update.get("semantic_dense_summary"):
        chunk_projection_update["retrieval_text"] = re.sub(
            r"\s+",
            " ",
            f"{chunk_projection_update.get('semantic_dense_summary', '')} {chunk_projection_update.get('retrieval_text', '')}",
        ).strip()[:1800]
...
def run_agentic_corpus_enrichment(
    *,
    ...
    for paragraph in selected_paragraphs:
        ...
        try:
            llm_payload, interpreter_stage = _chunk_interpreter_step(
                client,
                paragraph,
                page,
                document,
            ...
            chunk_assertions, validator_stage = _chunk_validator_step(
                _chunk_assertions(
                    paragraph=paragraph,
                    page=page,
                    document=document,
            ...
            assertions.extend(chunk_assertions)
            (
                updated_paragraph,
