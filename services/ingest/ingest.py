...
from services.ingest.chunk_processing import build_structural_chunks, finalize_structural_chunks
        ...
        for member in pdf_members:
            ...
            for page_num, page_text in enumerate(page_texts):
                ...
                if not structural_chunks:
                    structural_chunks = build_structural_chunks(doc_type="other", page_text=safe_page_text)
                chunk_ids_by_local_key: Dict[str, str] = {}
                chunk_ids_in_order: List[str] = []
                for chunk_index, chunk_spec in enumerate(structural_chunks):
                    paragraph_id = _stable_id(
                        "para",
                        page_id,
                        chunk_index,
                        chunk_spec.char_start,
                        chunk_spec.char_end,
                        chunk_spec.text[:1400],
                    )
                    chunk_ids_in_order.append(paragraph_id)
                    chunk_ids_by_local_key[f"__index__:{chunk_index}"] = paragraph_id
                    if chunk_spec.local_key:
                        chunk_ids_by_local_key[chunk_spec.local_key] = paragraph_id
                structural_chunks = finalize_structural_chunks(structural_chunks, chunk_ids_by_local_key)
                for chunk_index, chunk_spec in enumerate(structural_chunks):
                    ...
                    paragraph_class = (
                        chunk_spec.paragraph_class
                        or ("case_excerpt" if doc_type == "case" else "article_clause" if chunk_refs["article_refs"] else "body")
                    )
                    paragraph_id = chunk_ids_in_order[chunk_index]
                    section_kind = chunk_spec.section_kind or (
                        "definition"
                    ...
                    paragraphs.append(
                        {
                            ...
                            "chunk_type": chunk_spec.chunk_type or "paragraph",
                            "chunk_index_on_page": page_paragraph_index,
                            "char_start": max(0, chunk_spec.char_start),
                            "char_end": min(len(safe_page_text), chunk_spec.char_end),
                            "heading_path_full": heading_path,
                            "structural_level": chunk_spec.structural_level,
                            "parent_section_id": chunk_spec.parent_section_id,
                            "prev_chunk_id": chunk_spec.prev_chunk_id,
                            "next_chunk_id": chunk_spec.next_chunk_id,
                        ...
                        law_chunk_facets.append(
                            {
                                ...
                                "article_title": chunk_spec.article_title,
                                "part_ref": chunk_spec.part_ref,
                                "chapter_ref": chunk_spec.chapter_ref,
                                "section_ref": chunk_spec.section_ref or (chunk_refs["article_refs"][0] if chunk_refs["article_refs"] else None),
                                "schedule_number": chunk_spec.schedule_number,
                                "schedule_title": chunk_spec.schedule_title,
                                "definition_term": _compact_summary(chunk, source_label).split(" means ", 1)[0][:120] if " means " in chunk.lower() else None,
                        ...
                        case_chunk_facets.append(
                            {
                                ...
                                "court_level": "unknown",
                                "decision_date": issued_date,
                                "section_kind_case": chunk_spec.section_kind_case or "reasoning",
                    ...
                    chunk_search_documents.append(
                        {
                            ...
                            "money_values": chunk_refs["money_mentions"][:12],
                            "exact_terms": (
                                chunk_refs["article_refs"][:6]
                                + chunk_refs["law_refs"][:6]
                                + ([chunk_spec.article_title] if chunk_spec.article_title else [])
                                + ([chunk_spec.part_ref] if chunk_spec.part_ref else [])
                                + ([chunk_spec.chapter_ref] if chunk_spec.chapter_ref else [])
                            )[:16],
                            "search_keywords": (
                                tags[:6]
                                + chunk_refs["entities"][:4]
                                + [item for item in [chunk_spec.article_title, chunk_spec.part_ref, chunk_spec.chapter_ref, chunk_spec.section_ref] if item]
                            )[:16],
                            "version_lineage_id": canonical_doc_id,
                            "canonical_concept_id": canonical_concept_id,
                            "historical_relation_type": chunk_historical_relation_type,
                            ...
                            "chapter_ref": chunk_spec.chapter_ref,
                            "article_title": chunk_spec.article_title,
                            "section_kind_case": chunk_spec.section_kind_case,
                        }
                    )
