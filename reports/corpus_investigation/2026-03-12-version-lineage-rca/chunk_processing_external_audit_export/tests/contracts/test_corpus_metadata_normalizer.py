...
def test_metadata_normalization_records_field_level_page_provenance() -> None:
    payload = _payload()
    ...
    metadata = result["updated_documents"]["law-1"]["processing"]["metadata_normalization"]
    field_evidence = metadata["field_evidence"]
    assert field_evidence["title_raw"]["source_page_ids"] == ["employment-law_0"]
    assert field_evidence["title_raw"]["page_numbers_0"] == [0]
    assert field_evidence["law_number"]["section"] == "type_specific_document"

    chunk_projection = result["updated_chunk_projections"]["law-chunk-1"]
    assert chunk_projection["document_field_evidence"]["title_normalized"]["source_page_ids"] == ["employment-law_0"]
