...
def _page_window_evidence(pages: List[Dict[str, Any]]) -> Dict[str, Any]:
    source_page_ids = [str(page.get("source_page_id") or "").strip() for page in pages if str(page.get("source_page_id") or "").strip()]
    page_numbers_0 = [
        int(page.get("page_num", 0) or 0)
        for page in pages
        if page.get("page_num") is not None
    ]
    page_numbers_1 = [value + 1 for value in page_numbers_0]
    return {
        "source_page_ids": source_page_ids,
        "page_numbers_0": page_numbers_0,
        "page_numbers_1": page_numbers_1,
        "evidence_kind": "title_page_window",
    }


def _field_evidence_map(
    *,
    ...
    review: Dict[str, Any],
    court_normalization: Dict[str, Any],
) -> Dict[str, Dict[str, Any]]:
    window = _page_window_evidence(pages[:2])
    sections = {
        "canonical_document": canonical_document,
        "type_specific_document": type_specific_document,
        "processing_candidates": processing_candidates,
        "review": review,
        "court_normalization": court_normalization,
    }
...
def run_corpus_metadata_normalization(
    *,
    ...
    for document in documents:
        ...
        field_evidence = _field_evidence_map(
            pages=pages_for_doc,
        ...
        document_stage_runs[document_id] = {
            "title_page_normalizer": _stage_result(
                ...
                payload={
                    ...
                    "source_page_ids": envelope.get("context", {}).get("source_page_ids", []),
                    "field_evidence_keys": sorted(field_evidence.keys()),
    ...
    for group_key, group_docs in case_groups.items():
        ...
        for item in group_docs:
            ...
            source_page_ids = [
                str(page_id)
                for page_id in item.get("source_page_ids", [])
            ...
            processing["case_relation_resolution"] = {
                ...
                "field_evidence": {
                    "case_family_id": {
                        "source_page_ids": source_page_ids,
                        "evidence_kind": "case_title_window",
                        "field_name": "case_family_id",
                    },
                    "document_role_confirmed": {
                        "source_page_ids": source_page_ids,
                        "evidence_kind": "case_title_window",
                        "field_name": "document_role_confirmed",
                    },
                },
                "updated_at": _utcnow().isoformat(),
            }
